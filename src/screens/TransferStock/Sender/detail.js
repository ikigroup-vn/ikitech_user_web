...
class DetailImportStock extends Component {
    ...
    goBack = () => {
        ...
        var params = ""
        if (tabIndex) {
            params = params + `?tab-index=${tabIndex}`
            history.replace(`/transfer_stocks/index/${store_code}${params}`)
        }
        else {
            history.replace(`/transfer_stocks/index/${store_code}?tab-index=1`)
        }
    }
    render() {
        const { store_code, id } = this.props.match.params
                                                ...
                                                <div style={{ marginBottom: "10px" }}>
                                                    {
                                                        transfer_stock_items.map((item, index) => (
                                                            <ItemDetail getRefund={this.getRefund} id={id} store_code={store_code} index={index} listItems={transfer_stock_items} status={itemImportStock.status} check={check} listItem={item} />
                                                        ))
                                                    }
                                                </div>
                                                {/* <div class="">
...
const mapDispatchToProps = (dispatch, props) => {
    return {
        ...
        fetchDetailTransferStock: (store_code, branch_id, id) => {
            dispatch(TransferStockAction.fetchDetailTransferStock(store_code, branch_id, id))
        },
        changeStatus: (store_code, branch_id, id, data) => {
            dispatch(TransferStockAction.changeStatus(store_code, branch_id, id, data))
        }
    }
}
