            ...
            <div class="bootstrap-autocomplete dropdown-menu show">
                {
                    suggestions.map((item, index) => {
                        ...
                        text = <div>{text.substring(0, item.p1,)}<b>{text.substring(item.p1, item.p2)}</b>{text.substring(item.p2)}</div>

                        return <a class="dropdown-item"
                            onClick={() => this.suggestionSeletected(item)}
                            className={this.state.indexSelect == index ? "dropdown-item active" : "dropdown-item"}
                        >{text}</a>
