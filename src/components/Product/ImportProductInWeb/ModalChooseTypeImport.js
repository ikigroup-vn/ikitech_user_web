...
class ModalChooseTypeImport extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (Object.entries(nextProps).length > 0 && !shallowEqual(messageImport, nextProps.messageImport)) {
      setOpenModal(false);
    }
    ...
    reader.onload = (evt) => {
      ...
      dataXlsxEmptyTitle.forEach((product, index) => {
        ...
        if (product[16]?.toString().toLowerCase().trim() === 'không') {
          ...
          newProduct['status'] = product[13]?.toString().toLowerCase().trim() === 'hiện' ? 0 : -1;
          ...
          if (product[9]?.includes('(VND)')) {
            ...
            newProduct['money_amount_collaborator'] = Number(money_amount_collaborator);
            newProduct['type_share_collaborator_number'] = 1;
          } else if(product[9]?.includes('(%)')) {
            const percent_collaborator = product[9]?.split("(%)")?.[0];
            newProduct['percent_collaborator'] = Number(percent_collaborator) > 100 ? 100 : Number(percent_collaborator);
          ...
          newProduct['list_category'] = product[5]
            ? product[5]
                ...
                .reduce((prevCategory, currentCategory) => {
                  ...
                  newCategory.childs = !childs ? [] : childs.split(',').map((childCategory) => childCategory);
          ...
          newProduct['attribute_search_children'] = product[7] ? product[7]?.toString().split(',') : [];
          ...
          newProductHasDistribute['status'] = product[13]?.toString().toLowerCase().trim() === 'hiện' ? 0 : -1;
          ...
          if (product[9]?.includes('(VND)')) {
            ...
            newProductHasDistribute['money_amount_collaborator'] = Number(money_amount_collaborator);
            newProductHasDistribute['type_share_collaborator_number'] = 1;
          } else if(product[9]?.includes('(%)')) {
            const percent_collaborator = product[9]?.split("(%)")?.[0];
            newProductHasDistribute['percent_collaborator'] = Number(percent_collaborator) > 100 ? 100 : Number(percent_collaborator);
          ...
          newProductHasDistribute['images'] = !product[22] ? [] : product[22].split(',');
          // Hanlde Categories
          ...
          // Handle Attribute Search
          newProductHasDistribute['attribute_search_children'] = product[7] ? product[7]?.toString().split(',') : [];
          newProductHasDistribute['weight'] = product[8] ? Number(product[8]) : 0;
          ...
          const nameProductDistributeTemp = product[19] ? product[19]?.toString().split(',')[0] : '';
          const nameProductSubDistributeTemp = product[19] ? product[19]?.toString().split(',')[1] : '';
          const imagesProductDistributeTemp = !product[22] ? '' : product[22]?.split(',');
          ...
          if (nameElementDistribute !== nameProductDistributeTemp) {
            ...
            newDistributes[0]['element_distributes'].push({
              ...
              price: nameProductDistributeTemp ? (!product[20] ? 0 : Number(product[20])) : 0,
              import_price: nameProductDistributeTemp ? (!product[21] ? 0 : Number(product[21])) : 0,
              image_url: imagesProductDistributeTemp ? imagesProductDistributeTemp[0] : '',
              sub_element_distributes: [],
            });
            nameElementDistribute = nameProductDistributeTemp;
          }
          if (nameProductSubDistributeTemp) {
            newDistributes[0]['element_distributes'][positionDistributeProduct]['sub_element_distributes'].push({
        ...
        if (
          ...
          newProduct['distributes'] = [];
        } else if (isDistributeProduct === true && index === dataXlsxEmptyTitle.length - 1) {
          newProductHasDistribute['distributes'] = newDistributes;
          newProducts.push({ ...newProductHasDistribute });
          newDistributes = [];
          isDistributeProduct = false;
      ...
      >
        <ModalChooseTypeImportStyles>
          <div className="modalImport__content">
            <input type="file" id="import_file_iki" hidden onChange={this.handleChangeFile} />
            <div className="modalImport__override">
              ...
              />
              <label for="overridePermission">Cho phép bỏ qua các sản phẩm trùng tên</label>
            </div>
            <div className="modalImport__btn">
              <button className="btn btn-outline-success" onClick={this.handleImportIki}>
                Import từ IKITECH
              </button>
              <button className="btn btn-outline-primary" onClick={this.handleImportOutSide}>
                Import từ bên ngoài
              </button>
            </div>
          </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(ModalChooseTypeImport);
