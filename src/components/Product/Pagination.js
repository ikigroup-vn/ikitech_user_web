...
class Pagination extends Component {
  ...
  passPaginationProduct = (page) => {
    ...
    passNumPage(page);
    history.push(`/product/index/${store_code}?page=${page}${params}&is_selected=true&get_by_branch=true`);
    const branch_id = getBranchId();
    const branch_ids = getBranchIds();
    const branchIds = branch_ids ? branch_ids : branch_id;
    this.props.fetchAllProductV2(store_code, branchIds, page, params + "&is_selected=true&get_by_branch=true");
  };
  passPagination = (page) => {
    var {
      store_code,
