    ...
    showData = (historyPayment) => {
        ...
        if (historyPayment.length > 0) {
            historyPayment.forEach((data, index) => {
                ...
                var status = data.status == 0 || data.status == 1  ? "Đã hủy" : "Đã thanh toán"
                var _status = data.status == 0 || data.status == 1  ? "danger" : "success"
                var address_default = ""
                ...
                result.push(
                    <React.Fragment>
                        <tr class="sub-container hover-product">
                            ...
                            <td>                                                    {data.collaborator.first_and_last_name}
                            </td>
                            <td> {data.collaborator.account_number} -{" "}
                                                    {data.collaborator.bank}{" "}</td>
                            <td>{moment(data.created_at).format("DD-MM-YYYY")}</td>

                            <td className = {_status}>{moment(data.updated_at).format("DD-MM-YYYY")}</td>
