...
class Table extends Component {
  ...
  passEditFunc = (e, id , name ,image,isShowHome) => {
    this.props.handleUpdateCallBack({id : id , name : name , image_url :image,is_show_home :isShowHome});
    e.preventDefault();
  };
  ...
  deleteChild = (e, id ,idChild, name) => {
    ...
    this.props.handleDeleteChild({title : "Danh mục con" , id : id ,idChild: idChild, name : name });
    e.preventDefault();
  };
  createChild = (e,id) =>{
  ...
  editChild = (e, id,idChild,name,image_url) =>{
    this.props.handleUpdateChild({image:image_url, id : id ,idChild: idChild, name : name });
    e.preventDefault();    
  }
   onSortEnd = (oldIndex, newIndex) => {
    ...
    this.props.sortCategory(this.props.store_code , {ids : listId , positions : listPosition})
     this.setState({
      listArr: listArr,
            ...
            <SortableItem key={data.id}>
              <div class="resp-table-row" style={{width:"100%"}}>
                ...
                <div class="table-body-cell table-custom" style={{width:"35%"}}>
                  ...
                  <a
                    style={{ marginLeft: "10px",color:"white"}}
                    onClick={(e) => this.passDeleteFunc(e, data.id, data.name)}
                    data-toggle="modal"
                ...
                <div class="table-body-cell" style = {{position:"relative"}}>  
                      {data.category_children.map((data1,index) =>{
                        var image_url_child =
                        data1.image_url == null || data1.image_url == "" ?  Env.IMG_NOT_FOUND : data1.image_url
                      return (
                        <>
                        <div className="wrap-conten-child" style={{display:"flex",padding:"3px",borderBottom:"1px solid #bdbdbd", justifyContent:"space-between", marginBottom:"5px"}}>
                          ...
                          <div class="button-category btn-group">
                            ...
                            <a
                              style={{ marginLeft: "10px",color:"white"}}
                              onClick={(e) => this.deleteChild(e, data.id, data1.id, data1.name)}
                              data-toggle="modal"
                    ...
                    <div className="create-category-child" style={{float:"right"}}>
                            <a
                              style={{ marginLeft: "10px", width:"28px"}}
                              onClick={(e) => this.createChild(e, data.id)}
                              data-toggle="modal"
                              data-target="#createModalChild"
                              class={`btn btn-info btn-icon-split btn-sm show ${_delete == true ? "show" : "hide"}`}
                            >
...
<div id="resp-table">
      ...
      <div style = {{fontWeight : "500"}} class="table-body-cell">STT</div>
      <div style = {{fontWeight : "500"}} class="table-body-cell">Hình ảnh</div>
      <div style = {{fontWeight : "500"}} class="table-body-cell">Tên danh mục</div>
      <div style = {{fontWeight : "500"}} class="table-body-cell">Danh mục con</div>
        </div>
        <SortableList
