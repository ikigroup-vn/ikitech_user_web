...
class ListProduct extends Component {
  ...
  showData = (products) => {
    ...
    if (products.length > 0) {
      result = products.map((data, index) => {
        var status_name =
          data.status == 0 ? 'Còn hàng' : data.status == 1 ? 'Đã ẩn' : data.status == 2 ? 'Hết hàng' : null;
  ...
  render() {
    var { products, store_code, fetchAllProductV2 } = this.props;
    var { searchValue } = this.state;
    const branch_id = localStorage.getItem('branch_id');
    return (
      <div
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content" style={{ maxHeight: '630px' }}>
            ...
            <div class="group-pagination_flex col-xs-12 col-sm-12 col-md-12 col-lg-12">
              <Pagination
                fetchAllProductV2={fetchAllProductV2}
                ...
                branch_id={branch_id}
              />
              <button type="button" class="btn btn-default pagination-btn" data-dismiss="modal">
                Đóng
              </button>
            </div>
          </div>
