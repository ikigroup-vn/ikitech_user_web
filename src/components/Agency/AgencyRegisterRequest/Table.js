...
class Table extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      (!shallowEqual(prevProps.agencies, this.props.agencies) &&
        prevProps.agencies.length == 0) ||
      prevProps.tabId != 1 ||
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (!shallowEqual(this.props.agencies, nextProps.agencies)) {
        ...
        return (
          <React.Fragment>
            <tr class="sub-container hover-product">
              ...
              <td>
                {(this.props.agencies.current_page - 1) *
                  Number(this.props.agencies.per_page) +
                  index +
                  1}
              </td>{" "}
                ...
                {data2.status == 0 || data2.status == 3 ? (
                  <div>
                    <button
                      style={{ margin: "2px 0" }}
                      onClick={() => {
                        this.handleAgencyRegisterRequest(data2.id, 1);
                      }}
  ...
  render() {
    var agencies =
      typeof this.props.agencies.data == "undefined"
        ? []
    ...
    return (
      <ListAgencyStyles class="" style={{ overflow: "auto" }}>
        ...
        <table class="table table-border">
          ...
          <tbody>{this.showData(agencies)}</tbody>
        </table>
      </ListAgencyStyles>
    );
  }
