...
import * as employeeListAction from "../../actions/dashboard";
    ...
    return (
      <nav aria-label="Page navigation" className="float-pagination">
        <ul class="pagination  tab-pagination pg-blue">
          {this.showData(this.props.employeeList.links)} 
        </ul>
      </nav>
    );
  }
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    fetchEmployeeList: (store_code, page, params) => {
      dispatch(
        employeeListAction.fetchEmployeeList(store_code, page, params)
      );
    },
  };
};
