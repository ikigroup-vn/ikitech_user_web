...
class ChartFinance extends Component {
    ...
    componentWillReceiveProps(nextProps){
        if(this.props.reportProfit !== nextProps.reportProfit || this.props.compareProfit !== nextProps.compareProfit){
            var chartDataState_prime = { ...this.state.chartDataPrime }
            const {datePrime,dateCompare} = this.state
            const showDateChoose = `${moment(datePrime.from, "YYYY-MM-DD").format("DD/MM/YY")} - ${moment(datePrime.to, "YYYY-MM-DD").format("DD/MM/YY")}` 
            const showDateCopare = `${moment(dateCompare.from, "YYYY-MM-DD").format("DD/MM/YY")} - ${moment(dateCompare.to, "YYYY-MM-DD").format("DD/MM/YY")}`
...
const mapDispatchToProps = (dispatch, props) => {
    return {
        ...
        fetchReportProfitCompare: (store_code, branch_id, params) => {
            dispatch(reportAction.fetchReportProfitCompare(store_code, branch_id, params))
        }
    }
}
