            ...
            <div class="card card-item-pos" style={{ marginBottom: "10px" }} key={index}>
                <div class="card-body" style={{ padding: "0", position: "relative" }}>
                    <div className='wrap-item' style={{ display: "flex", fontSize: "1rem", justifyContent: 'space-between', alignItems: "center" }}>
                        ...
                        <img src={item.product.images.length > 0 ? item.product.images[0].image_url : Env.IMG_NOT_FOUND_2}
                            className="img-responsive image-product" alt="Image" width="50px" height="50px"
                        />

                        <div className='info-product' style={{ width: "35%", fontWeight: "400", display: "flex", flexDirection: "column" }}>
                        ...
                        <div className='cost' style={{ width: "15%", fontWeight: "400" }}>
                            {format(Number(item.item_price))}
                            {
                                ...
                                <div className=' past-price-pos'>  {format(Number(item.before_discount_price))}</div>}
                        </div>
                    ...
                    <i
                        ...
                        onClick={() => this.handleDelete(item.list_cart_id, item.product.id, item.id, item.distributes_selected)}></i>
                </div>
            </div>
        )
    }
