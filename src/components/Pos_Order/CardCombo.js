...
class CardProduct extends Component {
      ...
      <div className="discount-product-card">
        <div className="image">
          ...
          {product.product_discount && (
            <div class="discount-tag">{product.product_discount.value}%</div>
          )}
        </div>
        ...
        <div className="price">
          {product.min_price == product.max_price
            ? format(Number(product.min_price))
            : `${format(Number(product.min_price))}- ${format(
  ...
  render() {
    ...
    // }
    const { name, value, type, products, end, set_limit_amount, id, key } =
      this.props;
    ...
    return (
      <div className="combo-card">
          ...
          <div className="info">
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              {products?.length >= 3 && (
                <a
                  className="view"
                  onClick={() => this.showDetail({ products, name, id })}
                >
        ...
        <div
          className="products"
          onClick={() => {
            this.props.addComboInCart({ combo_id: id });
          }}
        >
          {products?.length > 0 &&
            products?.map(
              (v, i) =>
                i < 3 && (
                  <div className="card-wraper col-4" key={i}>
