...
import { compressed, isEmpty } from "../../ultis/helpers";
...
class BackgroundModal extends Component {
  ...
  componentDidMount() {
    ...
    prizeCodeApi
      .getConfig(store_code)
      .then((res) => {
        this.setState({
          mainImage: res.data.data.background_prize_code,
        });
      })
      .catch(() => {});
  }
  ...
  onSave = async (e) => {
    e.preventDefault();
    const { store_code } = this.props;
    if (!this.state.mainImage || !isEmpty(this.state.mainImage)) {
      this.props.showError({
        ...
        alert: {
          ...
          title: "Lỗi",
          disable: "show",
          content: "Ảnh không được để trống",
        },
      });
      return;
    }
    ...
    const data = {
      background_prize_code: this.state.mainImage,
    };
    this.props.showLoading();
    prizeCodeApi
      .updateBackgroundPrizeCode(store_code, data)
      .then((res) => {
      ...
      >
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            >
              <h4 class="modal-title">Cập nhật background</h4>
...
export default connect(null, mapDispatchToProps)(BackgroundModal);
